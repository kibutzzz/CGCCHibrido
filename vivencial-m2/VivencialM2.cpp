...
void triangulateAndAppend(const vector<int>& faceVerts,
                          const vector<glm::vec3>& vertices, glm::vec3 color,
...
ObjModel loadModel(const string& name, const string& filePath,
                   glm::vec3 position, float scale, glm::vec3 color) {
  ObjModel obj;
  obj.name = name;
...
void drawModels(GLint modelLoc) {
  for (int i = 0; i < (int)models.size(); i++) {
    ...
    if (i == selectedModel) model = glm::scale(model, glm::vec3(1.05f));
...
void printModeHelp(Mode m) {
  switch (m) {
    ...
    case Mode::TRANSLATE:
      cout
          << "[Modo TRANSLACAO]  A/D: eixo X  |  W/S: eixo Z  |  I/J: eixo Y\n";
      break;
    case Mode::SCALE:
      cout << "[Modo ESCALA]  X/Y/Z: escala por eixo  |  U: uniforme  |  "
              "Shift: diminuir\n";
      break;
  }
}
...
int main() {
  ...
  GLFWwindow* window =
      glfwCreateWindow(WIDTH, HEIGHT, "Vivencial M2 - Selecao e Transformacoes",
                       nullptr, nullptr);
  ...
  models.push_back(loadModel("Plane", modelsDir + "1405 Plane.obj",
                             glm::vec3(-0.6f, 0.0f, 0.0f), 0.4f,
                             glm::vec3(0.4f, 0.7f, 1.0f)));
  models.push_back(loadModel("House", modelsDir + "PUSHILIN_house.obj",
                             glm::vec3(0.0f, -0.2f, 0.0f), 0.4f,
                             glm::vec3(0.9f, 0.75f, 0.5f)));
  models.push_back(loadModel(
      "Lamborghini", modelsDir + "Lamborghini_Aventador.obj",
      glm::vec3(0.6f, 0.0f, 0.0f), 0.4f, glm::vec3(1.0f, 0.4f, 0.1f)));
  ...
  switch (key) {
    ...
    case GLFW_KEY_X:
      if (currentMode == Mode::ROTATE)
        ...
        obj.scale.x =
            max(SCALE_MIN, obj.scale.x + (shift ? -SCALE_STEP : SCALE_STEP));
      break;
    case GLFW_KEY_Y:
      if (currentMode == Mode::ROTATE)
        ...
        obj.scale.y =
            max(SCALE_MIN, obj.scale.y + (shift ? -SCALE_STEP : SCALE_STEP));
      break;
    case GLFW_KEY_Z:
      if (currentMode == Mode::ROTATE)
        ...
        obj.scale.z =
            max(SCALE_MIN, obj.scale.z + (shift ? -SCALE_STEP : SCALE_STEP));
      break;
