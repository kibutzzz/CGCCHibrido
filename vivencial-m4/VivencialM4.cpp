...
GLuint uploadMeshToGPU(const vector<GLfloat>& vBuffer, int& nVertices)
{
    ...
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, STRIDE * sizeof(GLfloat), (GLvoid*)(OFFSET_NORMAL   * sizeof(GLfloat)));
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, STRIDE * sizeof(GLfloat), (GLvoid*)(OFFSET_TEXCOORD * sizeof(GLfloat)));
...
Material loadMTL(const string& filePath)
{
    ...
    while (getline(file, line)) {
        ...
        if      (token == "Ka") { ss >> mat.ka.r >> mat.ka.g >> mat.ka.b; hasKa = true; }
        else if (token == "Kd") { ss >> mat.kd.r >> mat.kd.g >> mat.kd.b; hasKd = true; }
        else if (token == "Ks") { ss >> mat.ks.r >> mat.ks.g >> mat.ks.b; hasKs = true; }
        else if (token == "Ns") { ss >> mat.ns; }
        else if (token == "map_Kd") { ss >> mat.texName; }
    }
...
array<PointLight, 3> computeThreePointLights(glm::vec3 objectCenter, float objectScale)
{
    float r = 4.0f * objectScale;
    return {{
        { objectCenter + glm::vec3( r * 0.8f,  r,        -r * 0.5f), glm::vec3(1.0f, 0.95f, 0.85f), 1.6f },
        { objectCenter + glm::vec3(-r,          r * 0.5f, -r * 0.5f), glm::vec3(0.8f, 0.85f, 1.0f),  0.7f },
        { objectCenter + glm::vec3( 0.0f,       r * 0.5f,  r),        glm::vec3(1.0f, 1.0f,  1.0f),  0.9f },
    }};
}

void uploadLightUniforms(GLuint shaderID, const array<PointLight, 3>& lights, const bool enabled[3])
{
    for (int i = 0; i < 3; i++) {
        string base = "lights[" + to_string(i) + "].";
        glUniform3fv(glGetUniformLocation(shaderID, (base + "position").c_str()),   1, glm::value_ptr(lights[i].position));
        glUniform3fv(glGetUniformLocation(shaderID, (base + "lightColor").c_str()), 1, glm::value_ptr(lights[i].color));
        glUniform1f (glGetUniformLocation(shaderID, (base + "intensity").c_str()),  lights[i].intensity);
        glUniform1i (glGetUniformLocation(shaderID, (base + "enabled").c_str()),    enabled[i] ? 1 : 0);
    }
}

glm::mat4 buildModelMatrix(glm::vec3 position, float scale, float ax, float ay, float az)
{
    glm::mat4 model(1.0f);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Phong Viewer M4 - 3-Point Lighting", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 model = buildModelMatrix(objPosition, objScale, angleX, angleY, angleZ);
